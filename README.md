...
- HTML 排版畫布：文字方框可拖曳移動、右下角可拉大小、溢出標紅、id 標籤、量測模式（框選面板內緣後一鍵套用成方框座標）、概念圖疊圖與差異模式、格線與成品預覽、鄰近文字框邊緣／中心吸附（按住 Alt 自由放置）、自動草稿保存與重新載入、一鍵複製 JSON。
- `-Detach` 直接開本機網頁：父指令立即返回，隱藏 worker 持續提供 loopback 服務並開瀏覽器；按「儲存並回傳 agent」就把改好的設定寫回檔案、清除草稿、印出每個區塊改了哪些欄位，不必手動複製貼上 JSON。agent 可輪詢回傳物件中的 `ResultPath`。`不存離開` 或逾時不會寫入主 config。短暫手動檢查仍可用 `-Serve`。
...
- 編輯輔助控制項、autosave 草稿重載、正式儲存後清草稿、以及 detached serve round-trip 都通過。
