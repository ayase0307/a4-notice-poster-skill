...
$fontNames = @([System.Drawing.FontFamily]::Families.Name | Sort-Object -Unique)
...
.shape,.text{position:absolute}.text{display:flex;white-space:pre-line;overflow:visible;line-height:1.18}.selected{outline:3px solid #1d6ee8;outline-offset:2px}
...
<h1>A4 公告海報排版畫布</h1><p class="hint">選擇文字區塊後可暫時比較本機字型與位置。確認後下載 JSON，交由 Codex 套回正式設定。</p>
...
const fields=['text','font','size','color','x','y','width','height','align','valign'];const el=Object.fromEntries(fields.map(id=>[id,document.querySelector('#'+id)]));
...
function loadBlock(){const t=config.texts[Number(blockSelect.value)||0];if(!t)return;el.text.value=t.text;el.font.value=t.fontFamily||config.fontFamily;['size','x','y','width','height','align','valign'].forEach(k=>el[k].value=t[k]);el.color.value=cssColor(t.color).slice(0,7)}
function update(){const t=config.texts[Number(blockSelect.value)||0];if(!t)return;t.text=el.text.value;t.fontFamily=el.font.value;['size','x','y','width','height'].forEach(k=>t[k]=Number(el[k].value));['color','align','valign'].forEach(k=>t[k]=el[k].value);render()}
