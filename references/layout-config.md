...
1. Serve the canvas. `-Serve` starts a loopback web server, opens the default browser at it, and blocks until the reviewer presses a button, so the edited layout comes straight back to you. Always pass `-ConceptPath` so the approved concept can be compared against the typesetting:
   ...
   Keep `-TimeoutMinutes` under whatever command timeout the agent harness enforces, or run the command in the background. If the harness kills the command first, the reviewer's press has nowhere to land and the config stays unchanged. The HTML file is always written before the server starts, so the reviewer can fall back to `複製 JSON` from that file.
