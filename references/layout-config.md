...
1. Serve the canvas. For agent-driven reviews, use `-Detach`: it starts a hidden loopback worker that survives the calling command, opens the browser, returns metadata immediately, and writes a result marker when the reviewer finishes. Blocking `-Serve` remains available for short manual checks and smoke tests. Always pass `-ConceptPath` so the approved concept can be compared against the typesetting:

   ```powershell
   ./scripts/build_poster_canvas.ps1 -ConfigPath ./poster-config.json -ConceptPath ./concept.png -Detach
   ```

   The command returns an object with `Url`, `ProcessId`, `ReadyPath`, and `ResultPath`. Poll `ResultPath`; on save it reports `status=saved`, machine-readable `changes`, and writes the reviewed config to `-ConfigPath`, or to `-SaveConfigPath`. `cancelled`, `timeout`, and `error` do not write the main config. Add `-Port` to pin the port, and `-NoBrowser` when no browser should be launched for you. Without `-Serve` or `-Detach` the script only writes the HTML file, which still works but makes the reviewer paste the JSON back by hand.

   Every edit also autosaves `<config>.canvas-draft.json`; reopening restores it, final save clears it, and `還原原始` deletes it. This protects work if the tab closes before Approval 3.
...
3. Correct placement directly on the canvas instead of guessing coordinates from the image. Drag a block to move it, drag the blue handle on its bottom-right corner to resize it, and nudge the selected block with the arrow keys by `1 px` or with `Shift` plus an arrow key by `10 px`. The box snaps to nearby text-box edges and centers within the configured distance; hold `Alt` for free placement. Toggle `格線` when spacing needs another reference, and use `成品預覽` to hide editor boxes, labels, handles, guides, grid, measurement overlays, and concept overlay. The `X`, `Y`, `寬度`, and `高度` fields update live.
...
6. Return the layout. Under serve mode, `儲存並回傳 agent` posts the edited config back, writes it to disk, clears the draft, and ends detached worker or blocking command; `不存離開` keeps the autosaved draft but does not write the main config. Without server mode, `複製 JSON` puts the config on the clipboard and `下載` writes `poster-config-reviewed.json`, and browser changes stay temporary unless localStorage retained them. The saved result names only fields and numbers: copy that the reviewer rewrote is reported as `text edited`, because Windows PowerShell writes redirected output in the system codepage. Read the saved config, which is UTF-8, for the wording.
